...
{ 	
	...
	ftruncate(fd,sizeof(mydomainbuf)+1);
	//FILE *filename;
	//sem_t *sem;
	int counter=0;
	int s;
	addr=(char*)mmap(0,sizeof(mydomainbuf), PROT_WRITE|PROT_READ,MAP_SHARED,fd,0);
	...
	*/
	...
	{	
		...
		status=getdomainname(mydomainbuf,(int)sizeof(mydomainbuf));   
    		...
    		memcpy(addr,mydomainbuf,sizeof(mydomainbuf));
		printf("%d) Written: %s\n",counter,mydomainbuf);
		if(sem_post(sem_writer)!=0) exit(1);		
		if(sem_wait(sem_read)!=0) exit(1);
		sleep(1);
		counter++;
